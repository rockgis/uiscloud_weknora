...
이 클라이언트는 다음과 같은 주요 기능 모듈을 포함합니다:
...
6. **Agent Q&A**: Agent 기반 지능형 Q&A 지원 (사고 과정, 도구 호출, 자기 반성 포함)
...
### 예제: 지식베이스 생성 및 파일 업로드

```go
// 지식베이스 생성
kb := &client.KnowledgeBase{
    Name:        "테스트 지식베이스",
    Description: "이것은 테스트 지식베이스입니다",
...
sessionRequest := &client.CreateSessionRequest{
    ...
    SessionStrategy: &client.SessionStrategy{
        ...
        FallbackResponse: "죄송합니다, 해당 질문에 답변할 수 없습니다",
        EmbeddingTopK:    5,
...
Agent Q&A는 도구 호출, 사고 과정 표시, 자기 반성을 지원하는 더욱 강력한 지능형 대화 기능을 제공합니다.

```go
// Agent 세션 생성
agentSession := apiClient.NewAgentSession(session.ID)

// 완전한 이벤트 처리를 포함한 Agent Q&A 수행
err := agentSession.Ask(context.Background(), "머신러닝 관련 지식을 검색하고 요점을 요약해주세요",
...
| `AgentResponseTypeToolCall` | 도구 호출 | Agent가 특정 도구 사용을 결정할 때 |
...
Agent 기능 테스트를 위한 대화형 커맨드라인 도구를 제공합니다:
...
더 많은 고급 사용 예제는 `agent_example.go` 파일을 참조하세요. 다음을 포함합니다:
- 기본 Agent Q&A
- 도구 호출 추적
- 지식 참조 캡처
- 전체 이벤트 추적
...
`example.go` 파일의 `ExampleUsage` 함수를 참조하세요. 클라이언트의 전체 사용 흐름을 보여줍니다.
