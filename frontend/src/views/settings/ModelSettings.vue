      ...
      <div class="builtin-models-info">
        <div class="info-box">
          <div class="info-header">
            ...
            <span class="info-title">내장 모델</span>
          </div>
        ...
        <div v-for="model in chatModels" :key="model.id" class="model-card" :class="{ 'builtin-model': model.isBuiltin }">
          <div class="model-info">
            <div class="model-name">
              {{ model.name }}
              <t-tag v-if="model.isBuiltin" theme="primary" size="small">내장</t-tag>
            </div>
...
const loadModels = async () => {
  ...
  try {
    ...
    MessagePlugin.error(error.message || '모델 목록 로드 실패')
  } finally {
    loading.value = false
  }
}
...
const editModel = (type: 'chat' | 'embedding' | 'rerank' | 'vllm', model: any) => {
  if (model.isBuiltin) {
    MessagePlugin.warning('내장 모델은 편집할 수 없습니다')
    return
  }
  currentModelType.value = type
