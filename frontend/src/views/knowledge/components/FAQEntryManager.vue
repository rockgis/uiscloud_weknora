          ...
          <div class="progress-bar-header">
            <div class="progress-left">
              ...
              <span class="progress-title">
                {{ importState.taskStatus.status === 'running' ? '가져오는 중...' : 
                   importState.taskStatus.status === 'success' ? '가져오기 완료' : 
                   importState.taskStatus.status === 'failed' ? '가져오기 실패' : '대기 중...' }}
              </span>
            </div>
            <div class="progress-right">
              <span class="progress-count">
                {{ importState.taskStatus.processed }}/{{ importState.taskStatus.total }} 개
              </span>
              <t-button
                ...
                >
                  ...
                  <div class="faq-card-footer">
                    ...
                    <div class="faq-card-status" @click.stop>
                      <!-- Temporarily hidden: recommendation toggle
                      <t-tooltip
        ...
        >
          <div class="settings-group">
            ...
            <div class="setting-row vertical">
              <div class="setting-info">
                <label>{{ $t('knowledgeBase.category') || '분류' }}</label>
                <p class="desc">{{ $t('knowledgeEditor.faq.tagDesc') || 'FAQ 항목에 분류를 선택해 주세요' }}</p>
              </div>
              <div class="setting-control">
                <t-select
                  ...
                  clearable
                  :placeholder="$t('knowledgeEditor.faq.tagPlaceholder') || '분류 선택'"
                />
              </div>
            </div>
          </div>
              ...
              <div class="faq-import-footer">
                ...
                >
                  {{ importState.taskStatus?.status === 'success' ? $t('common.close') : 
                     importState.taskStatus?.status === 'failed' ? '재시도' :
...
const tagDropdownOptions = computed(() => [
  { content: t('knowledgeBase.untagged') || '미분류', value: '' },
...
const tagSelectOptions = computed(() => [
  { label: t('knowledgeBase.untagged') || '미분류', value: '' },
...
const getTagName = (tagId?: string) => {
  if (!tagId) return t('knowledgeBase.untagged') || '미분류'
  return tagMap.value[tagId]?.name || (t('knowledgeBase.untagged') || '미분류')
}
...
const handleFaqMenuAction = (event: Event) => {
  ...
  if (detail.action === 'create') {
    ...
    if (selectedRowKeys.value.length === 0) {
      MessagePlugin.warning(t('knowledgeEditor.faq.selectEntriesFirst') || '먼저 조작할 FAQ 항목을 선택해 주세요')
    }
...
const openImportDialog = () => {
  if (importState.taskStatus?.status === 'running') {
    MessagePlugin.warning('가져오기 진행 중입니다. 완료 후 다시 시도해 주세요')
    return
  }
  stopPolling()
      ...
      complete: (results) => {
        try {
          ...
          results.data.forEach((row: any) => {
            ...
            const isDisabled = parseBooleanField(record['비활성화 여부'], false)
            payloads.push(
              normalizePayload({
                standard_question: record['질문'] || record['standard_question'] || record['question'] || '',
                answers: splitByDelimiter(record['봇 답변'] || record['answers']),
                similar_questions: splitByDelimiter(record['유사 질문'] || record['similar_questions']),
                negative_questions: splitByDelimiter(record['반례 질문'] || record['negative_questions']),
                tag_id: record['tag_id'] || '',
                tag_name: record['분류'] || record['tag_name'] || '',
                is_enabled: isDisabled !== undefined ? !isDisabled : undefined,
              }),
            )
          })
          resolve(payloads)
...
const parseExcelFile = async (file: File): Promise<FAQEntryPayload[]> => {
  ...
  return json.map((row) => {
    ...
    const isDisabled = parseBooleanField(normalizedRow['비활성화 여부'], false)
    return normalizePayload({
      standard_question: normalizedRow['질문'] || normalizedRow['standard_question'] || normalizedRow['question'] || '',
      answers: splitByDelimiter(normalizedRow['봇 답변'] || normalizedRow['answers']),
      similar_questions: splitByDelimiter(normalizedRow['유사 질문'] || normalizedRow['similar_questions']),
      negative_questions: splitByDelimiter(normalizedRow['반례 질문'] || normalizedRow['negative_questions']),
      tag_id: normalizedRow['tag_id'] || '',
      tag_name: normalizedRow['분류'] || normalizedRow['tag_name'] || '',
      is_enabled: isDisabled !== undefined ? !isDisabled : undefined,
    })
  })
}
...
const parseBooleanField = (value?: string, defaultValue: boolean = true): boolean | undefined => {
  ...
  if (normalized === 'TRUE' || normalized === '1' || normalized === '예' || normalized === 'YES') {
    return true
  }
  if (normalized === 'FALSE' || normalized === '0' || normalized === '아니오' || normalized === 'NO') {
    return false
  }
  return defaultValue
}
...
const exampleData: FAQEntryPayload[] = [
  ...
  {
    standard_question: '지식베이스를 어떻게 만드나요?',
    answers: ['"새 지식베이스" 버튼을 클릭하세요', '지식베이스 유형을 선택하고 정보를 입력하세요', '생성 완료 후 바로 사용할 수 있습니다'],
    similar_questions: ['지식베이스를 어떻게 만들어요?', '새 지식베이스를 어떻게 만드나요?'],
    negative_questions: [],
    tag_name: '사용 가이드',
  },
]
...
const downloadCSVExample = () => {
  const headers = ['분류(필수)', '질문(필수)', '유사 질문(선택-여러 개는 ##으로 구분)', '반례 질문(선택-여러 개는 ##으로 구분)', '봇 답변(필수-여러 개는 ##으로 구분)', '전체 답변 여부(선택-기본값 FALSE)', '비활성화 여부(선택-기본값 FALSE)', '추천 금지 여부(선택-기본값 False)']
...
const downloadExcelExample = () => {
  const worksheet = XLSX.utils.json_to_sheet(
    exampleData.map((item) => ({
      ...
      '유사 질문(선택-여러 개는 ##으로 구분)': item.similar_questions.join('##'),
      '반례 질문(선택-여러 개는 ##으로 구분)': item.negative_questions.join('##'),
      '봇 답변(필수-여러 개는 ##으로 구분)': item.answers.join('##'),
      '전체 답변 여부(선택-기본값 FALSE)': 'FALSE',
      '비활성화 여부(선택-기본값 FALSE)': 'FALSE',
      '추천 금지 여부(선택-기본값 False)': 'FALSE',
    })),
  )
...
const handleExportCSV = async () => {
  if (!props.kbId) {
    MessagePlugin.warning(t('knowledgeBase.selectKnowledgeBase') || '먼저 지식베이스를 선택해 주세요')
    return
  }
  ...
  try {
    ...
    document.body.removeChild(link)
    URL.revokeObjectURL(url)
    MessagePlugin.success(t('knowledgeEditor.faqExport.exportSuccess') || '내보내기 성공')
  } catch (error: any) {
    console.error('Export failed:', error)
    MessagePlugin.error(t('knowledgeEditor.faqExport.exportFailed') || '내보내기 실패')
  } finally {
