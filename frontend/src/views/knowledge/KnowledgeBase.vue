...
const fileTypeOptions = computed(() => [
  { content: t('knowledgeBase.allFileTypes') || '전체 유형', value: '' },
  ...
  { content: t('knowledgeBase.typeManual') || '직접 작성', value: 'manual' },
]);
...
const getTagName = (tagId?: string | number) => {
  if (!tagId && tagId !== 0) return t('knowledgeBase.untagged') || '미분류';
  const key = String(tagId);
  return tagMap.value[key]?.name || (t('knowledgeBase.untagged') || '미분류');
};
...
const getKnowledgeType = (item: any) => {
  ...
  if (item.type === 'manual') {
    return t('knowledgeBase.typeManual') || '직접 작성';
  }
  if (item.file_type) {
...
const handleKnowledgeTagChange = async (knowledgeId: string, tagValue: string) => {
  try {
    ...
    MessagePlugin.success(t('knowledgeBase.tagUpdateSuccess') || '분류가 업데이트되었습니다');
    loadKnowledgeFiles(kbId.value);
    loadTags(kbId.value);
  } catch (error: any) {
    MessagePlugin.error(error?.message || t('common.operationFailed'));
...
const handleFileUploaded = (event: CustomEvent) => {
  ...
  console.log('파일 업로드 이벤트 수신, 업로드된 지식베이스 ID:', uploadedKbId, '현재 지식베이스 ID:', kbId.value);
  if (uploadedKbId && uploadedKbId === kbId.value && !isFAQ.value) {
    console.log('현재 지식베이스와 일치, 파일 목록 새로 고침 시작');
...
const handleOpenURLImportDialog = (event: CustomEvent) => {
  ...
  console.log('URL 가져오기 다이얼로그 열기 이벤트 수신, 지식베이스 ID:', eventKbId, '현재 지식베이스 ID:', kbId.value);
...
const ensureDocumentKbReady = () => {
  if (isFAQ.value) {
    MessagePlugin.warning('현재 지식베이스 유형은 이 작업을 지원하지 않습니다');
    return false;
  }
  if (!kbId.value) {
    MessagePlugin.warning(t('knowledgeEditor.messages.missingId'));
...
const handleDocumentUpload = async (event: Event) => {
  ...
  for (const file of validFiles) {
    try {
      ...
      failCount++;
      let errorMessage = error?.error?.message || error?.message || "업로드 실패!";
...
const handleURLImportConfirm = async () => {
  ...
  if (!url) {
    MessagePlugin.warning(t('knowledgeBase.urlRequired') || 'URL을 입력해 주세요');
    return;
  }
  ...
  try {
    ...
    MessagePlugin.warning(t('knowledgeBase.invalidURL') || '유효한 URL을 입력해 주세요');
    return;
  }
  ...
  try {
    ...
    const isSuccess = responseData?.success || responseData?.code === 200 || responseData?.status === 'success' || (!responseData?.error && responseData);
    if (isSuccess) {
      MessagePlugin.success(t('knowledgeBase.urlImportSuccess') || 'URL 가져오기 성공!');
      urlDialogVisible.value = false;
      urlInputValue.value = '';
    } else {
      let errorMessage = t('knowledgeBase.urlImportFailed') || "URL 가져오기 실패!";
      if (responseData?.error?.message) {
        errorMessage = responseData.error.message;
      } else if (responseData?.message) {
        errorMessage = responseData.message;
      ...
      if (responseData?.code === 'duplicate_url' || responseData?.error?.code === 'duplicate_url') {
        errorMessage = t('knowledgeBase.urlExists') || "이 URL은 이미 존재합니다";
      }
      MessagePlugin.error(errorMessage);
    }
  } catch (error: any) {
    let errorMessage = error?.error?.message || error?.message || t('knowledgeBase.urlImportFailed') || "URL 가져오기 실패!";
      ...
      <div class="knowledge-main">
            ...
            <div class="tag-list">
              ...
              >
                <div class="tag-list-left">
                  ...
                  <span>{{ $t('knowledgeBase.untagged') || '미분류' }}</span>
                </div>
        ...
        <div class="tag-content">
          ...
          <t-dialog
            ...
            :header="$t('knowledgeBase.importURLTitle') || '웹페이지 가져오기'"
            :confirm-btn="{
              content: $t('common.confirm') || '확인',
              theme: 'primary',
              loading: urlImporting,
            }"
            :cancel-btn="{ content: $t('common.cancel') || '취소' }"
            @confirm="handleURLImportConfirm"
            @cancel="handleURLImportCancel"
            width="500px"
          >
            <div class="url-import-form">
              <div class="url-input-label">{{ $t('knowledgeBase.urlLabel') || 'URL 주소' }}</div>
              <t-input
                v-model="urlInputValue"
                :placeholder="$t('knowledgeBase.urlPlaceholder') || '웹페이지 URL을 입력하세요. 예: https://example.com'"
                clearable
                autofocus
                @keydown.enter="handleURLImportConfirm"
              />
              <div class="url-input-tip">{{ $t('knowledgeBase.urlTip') || '다양한 웹페이지 내용을 가져올 수 있으며, 시스템이 자동으로 텍스트를 추출하고 분석합니다' }}</div>
            </div>
          </t-dialog>
