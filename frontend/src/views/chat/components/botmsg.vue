...
const handleCopyAnswer = async () => {
    ...
    if (!content) {
        MessagePlugin.warning(t('chat.emptyContentWarning') || '현재 답변이 비어 있어 복사할 수 없습니다');
        return;
    }
    ...
    try {
        if (navigator.clipboard && navigator.clipboard.writeText) {
            ...
            MessagePlugin.success(t('chat.copySuccess') || '클립보드에 복사되었습니다');
        } else {
        ...
        MessagePlugin.error(t('chat.copyFailed') || '복사에 실패했습니다. 직접 복사해 주세요');
    }
};
...
const handleAddToKnowledge = () => {
    const content = getActualContent();
    if (!content) {
        MessagePlugin.warning(t('chat.emptyContentWarning') || '현재 답변이 비어 있어 지식베이스에 저장할 수 없습니다');
        return;
    }
    ...
    MessagePlugin.info(t('chat.editorOpened') || '편집기가 열렸습니다. 지식베이스를 선택한 후 저장해 주세요');
};
