...
const intermediateStepsSummary = computed(() => {
  ...
  const parts: string[] = [];
  if (searchCount > 0) {
    parts.push(`지식베이스 검색 <strong>${searchCount}</strong> 회`);
  }
  if (thinkingCount > 0) {
    parts.push(`사고 <strong>${thinkingCount}</strong> 회`);
  }
  if (toolCalls.length > 0) {
    const toolNames = toolCalls.map(name => {
      if (name === 'get_document_info') return '문서 조회';
      if (name === 'list_knowledge_chunks') return '지식 청크 보기';
      return name;
    });
    if (toolNames.length === 1) {
      parts.push(`${toolNames[0]} 호출`);
    } else {
      parts.push(`도구 호출: ${toolNames.join(', ')}`);
    }
  }
  
  if (parts.length === 0) {
    return `<strong>${intermediateStepsCount.value}</strong> 개의 중간 단계`;
  }
...
const getKbTooltipInnerHtml = (state: KbTooltipState): string => {
  ...
  return `<span class="tip-loading">로딩 중...</span>`;
};
...
const loadChunkDetails = async (chunkId: string) => {
  ...
  try {
    ...
    setKbCacheState(chunkId, { loading: false, error: '내용을 찾을 수 없습니다' });
  } catch (error: any) {
    console.error('Failed to load chunk details:', error);
    const errorMsg = error?.message || '로드 실패';
      ...
      const renderContent = (inner: string) => {
        tipElement.innerHTML = `
          <span class="t-popup__content">
            ${inner}
            <span class="tip-meta">청크 ID: ${shortChunkId}</span>
          </span>
        `;
      };
...
const getToolSummary = (event: any): string => {
  ...
  if (toolName === 'search_knowledge' || toolName === 'knowledge_search') {
    ...
    if (toolData?.fetched_chunks !== undefined) {
      const title = toolData?.knowledge_title || toolData?.knowledge_id || '문서';
    ...
    if (Array.isArray(steps)) {
      ...
      if (pending > 0) parts.push(`📋 대기 중 ${pending}`);
...
const getPlanStatusSummary = (event: any): string => {
  ...
  if (parts.pending > 0) textParts.push(`📋 대기 중 ${parts.pending}`);
...
const getSearchResultsSummary = (event: any): string => {
  ...
  if (count === 0) return `일치하는 내용을 찾을 수 없습니다`;
  
  // Build summary text
  let summary = '';
  const kbCount = toolData.kb_counts ? Object.keys(toolData.kb_counts).length : 0;
  if (kbCount > 0) {
    summary = `<strong>${kbCount}</strong> 개 파일에서 <strong>${count}</strong> 개 결과 발견`;
  } else {
    summary = `<strong>${count}</strong> 개 결과 발견`;
  }
  return summary;
};
...
const getGrepResultsSummary = (toolData: any): string => {
  ...
  if (totalMatches === 0) {
    return '일치하는 내용을 찾을 수 없습니다';
  }
  
  let summary = `<strong>${totalMatches}</strong> 곳 일치`;
  if (totalMatches > resultCount) {
    summary += `(${resultCount} 개 표시)`;
  }
...
const getToolDescription = (event: any): string => {
  ...
  if (toolName === 'search_knowledge' || toolName === 'knowledge_search') {
    return success ? '지식베이스 검색' : '지식베이스 검색 실패';
  } else if (toolName === 'web_search') {
...
const buildManualMarkdown = (question: string, answer: string): string => {
  const safeQuestion = question?.trim() || '(질문 내용 없음)';
  const safeAnswer = answer?.trim() || '(답변 내용 없음)';
  return `${safeAnswer}`;
};
...
const handleCopyAnswer = async (answerEvent: any) => {
  ...
  if (!content) {
    MessagePlugin.warning('현재 답변이 비어 있어 복사할 수 없습니다');
    return;
  }
  ...
  try {
    ...
    MessagePlugin.error('복사에 실패했습니다. 직접 복사해 주세요');
  }
};
...
const handleAddToKnowledge = (answerEvent: any) => {
  const content = getActualContent(answerEvent);
  if (!content) {
    MessagePlugin.warning('현재 답변이 비어 있어 지식베이스에 저장할 수 없습니다');
    return;
  }
  ...
  MessagePlugin.info('편집기가 열렸습니다. 지식베이스를 선택한 후 저장해 주세요');
};
</script>
