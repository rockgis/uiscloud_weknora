...
onMounted(() => {
  ...
  window.copyCodeBlock = (codeId: string) => {
    ...
    navigator.clipboard?.writeText(text).then(() => {
      MessagePlugin.success(t('common.copySuccess') || '복사되었습니다');
    }).catch(() => {
...
const loadOriginalContent = async () => {
  ...
  if (!isTextFile(fileType)) {
    MessagePlugin.warning(t('knowledgeBase.originalFileNotSupported') || '이 파일 형식은 원본 파일 보기를 지원하지 않습니다. 다운로드하여 확인해 주세요');
    return;
  }
  ...
  try {
    ...
    MessagePlugin.error(error?.message || t('knowledgeBase.loadOriginalFailed') || '원본 파일 내용 불러오기에 실패했습니다');
  } finally {
...
const getTypeLabel = () => {
  switch (props.details.type) {
    case 'url':
      return t('knowledgeBase.typeURL') || '웹페이지';
    case 'manual':
      return t('knowledgeBase.typeManual') || '수동 생성';
    case 'file':
      return props.details.file_type ? props.details.file_type.toUpperCase() : t('knowledgeBase.typeFile') || '파일';
    default:
      return '';
  }
};
...
const getContentLabel = () => {
  switch (props.details.type) {
    case 'url':
      return t('knowledgeBase.webContent') || '웹페이지 내용';
    case 'manual':
      return t('knowledgeBase.documentContent') || '문서 내용';
    case 'file':
    default:
      return t('knowledgeBase.fileContent') || '파일 내용';
  }
};
...
const getTimeLabel = () => {
  switch (props.details.type) {
    case 'url':
      return t('knowledgeBase.importTime') || '가져온 시간';
    case 'manual':
      return t('knowledgeBase.createTime') || '생성 시간';
    case 'file':
    default:
      return t('knowledgeBase.uploadTime') || '업로드 시간';
  }
};
...
const getChunkMeta = (item: any) => {
  ...
  const parts = [];
  if (item.char_count) {
    parts.push(`${item.char_count} ${t('knowledgeBase.characters') || '자'}`);
  }
  if (item.token_count) {
...
const handleDeleteQuestion = async (item: any, chunkIndex: number, question: GeneratedQuestion) => {
  if (!item || !item.id) {
    MessagePlugin.error(t('common.error') || '작업에 실패했습니다');
    return;
  }

  if (question.id.startsWith('legacy-')) {
    MessagePlugin.warning(t('knowledgeBase.legacyQuestionCannotDelete') || '이전 형식의 질문은 삭제할 수 없습니다. 질문을 다시 생성해 주세요');
    return;
  }

  const confirmDialog = DialogPlugin.confirm({
    header: t('common.confirmDelete') || '삭제 확인',
    body: t('knowledgeBase.confirmDeleteQuestion') || '이 질문을 삭제하시겠습니까? 삭제하면 해당 벡터 인덱스도 함께 제거됩니다.',
    confirmBtn: t('common.confirm') || '확인',
    cancelBtn: t('common.cancel') || '취소',
    onConfirm: async () => {
      confirmDialog.hide();
      deletingQuestion.value = { chunkIndex, questionId: question.id };
      try {
        await deleteGeneratedQuestion(item.id, question.id);
        MessagePlugin.success(t('common.deleteSuccess') || '삭제되었습니다');
        ...
        MessagePlugin.error(error?.message || t('common.deleteFailed') || '삭제에 실패했습니다');
      } finally {
  ...
  <div class="doc_content" ref="mdContentWrap">
    <t-drawer :visible="visible" :zIndex="2000" :closeBtn="true" @close="handleClose">
      ...
      <div v-else-if="details.type === 'url'" class="url_box">
        <span class="label">{{ $t('knowledgeBase.urlSource') || '출처 URL' }}</span>
      ...
      <div v-else-if="details.type === 'manual'" class="manual_box">
        <span class="label">{{ $t('knowledgeBase.documentTitle') || '문서 제목' }}</span>
        ...
        <div class="header-left">
          <div class="title-row">
            ...
            <span v-if="viewMode === 'chunks' && details.total > 0" class="chunk-count">
              {{ $t('knowledgeBase.chunkCount', { count: details.total }) || `총 ${details.total}개 청크` }}
            </span>
          </div>
          <div class="meta-row">
            ...
            >
              {{ viewMode === 'chunks' 
                ? (t('knowledgeBase.viewOriginal') || '원본 파일 보기') 
                : (t('knowledgeBase.viewChunks') || '청크 보기') }}
            </t-button>
          </div>
        </div>
      </div>
        ...
        <div v-else class="chunk-list">
          ...
          >
            <div class="chunk-header">
              <span class="chunk-index">{{ $t('knowledgeBase.segment') || '청크' }} {{ index + 1 }}</span>
              <div class="chunk-header-right">
                <t-tag 
                  ...
                  variant="light"
                >
                  {{ $t('knowledgeBase.questions') || '질문' }} {{ getGeneratedQuestions(item).length }}
                </t-tag>
            ...
            <div v-if="getGeneratedQuestions(item).length > 0" class="questions-section">
              <div class="questions-toggle" @click="toggleQuestions(index)">
                ...
                <span>{{ $t('knowledgeBase.generatedQuestions') || '생성된 질문' }} ({{ getGeneratedQuestions(item).length }})</span>
              </div>
