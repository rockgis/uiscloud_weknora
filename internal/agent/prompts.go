	...
	return []PlaceholderDefinition{
		{
			...
			Label:       "지식베이스 목록",
			Description: "지식베이스 이름, 설명, 문서 수, 최근 추가된 문서 등 정보를 테이블 형식으로 자동 포맷한 지식베이스 목록",
		},
		{
			Name:        "web_search_status",
			Label:       "웹 검색 모드 활성화 상태",
			Description: "웹 검색(web_search) 도구의 활성화 여부 상태 설명, 값은 Enabled 또는 Disabled",
		},
		{
			Name:        "current_time",
			Label:       "현재 시스템 시간",
			Description: "RFC3339 형식의 현재 시스템 시간, 모델이 실시간 인식에 활용",
		},
	}
}
