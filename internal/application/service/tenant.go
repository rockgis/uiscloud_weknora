...
func (s *tenantService) CreateTenant(ctx context.Context, tenant *types.Tenant) (*types.Tenant, error) {
	...
	apiKey, err := s.generateApiKey(0)
	if err != nil {
		logger.ErrorWithFields(ctx, err, map[string]interface{}{
			"tenant_name": tenant.Name,
		})
		return nil, errors.New("failed to generate initial API key: " + err.Error())
	}
	...
	apiKey, err = s.generateApiKey(tenant.ID)
	if err != nil {
		logger.ErrorWithFields(ctx, err, map[string]interface{}{
			"tenant_id":   tenant.ID,
			"tenant_name": tenant.Name,
		})
		return nil, errors.New("failed to generate API key: " + err.Error())
	}
	tenant.APIKey = apiKey
