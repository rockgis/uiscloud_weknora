...
// @Summary      지식 검색
// @Description  지식베이스에서 검색합니다 (LLM 요약 없음)
// @Tags         QA
// @Accept       json
// @Produce      json
...
// KnowledgeQA godoc
// @Summary      지식 QA
// @Description  지식베이스 기반 QA (LLM 요약 사용)，SSE 스트림 응답 지원
// @Tags         QA
// @Accept       json
// @Produce      text/event-stream
// @Param        session_id  path      string                   true  "세션 ID"
// @Param        request     body      CreateKnowledgeQARequest true  "QA 요청"
// @Success      200         {object}  map[string]interface{}   "QA 결과 (SSE 스트림)"
// @Failure      400         {object}  errors.AppError          "잘못된 요청 파라미터"
// @Security     Bearer
...
// AgentQA godoc
// @Summary      AgentQA
// @Description  에이전트 기반 지능형 QA，멀티턴 대화 및 SSE 스트림 응답 지원
// @Tags         QA
// @Accept       json
// @Produce      text/event-stream
// @Param        session_id  path      string                   true  "세션 ID"
