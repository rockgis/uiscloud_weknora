## 소개
uiscloud_weknora는 즉시 프로덕션 환경에 투입 가능한 엔터프라이즈급 RAG 프레임워크로, 지능형 문서 이해 및 검색 기능을 구현합니다. 이 시스템은 모듈화 설계를 채택하여 문서 이해, 벡터 저장, 추론 등의 기능을 분리합니다.
...
## PipeLine
uiscloud_weknora는 문서를 처리하기 위해 여러 단계를 거칩니다: 삽입 -> 지식 추출 -> 인덱싱 -> 검색 -> 생성. 전체 파이프라인은 다양한 검색 방법을 지원합니다.
